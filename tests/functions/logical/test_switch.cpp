...
TEST_F(SwitchFunctionTest, BasicMatch_ReturnsCorrectResult) {
    auto result = callSwitch({Value(2.0), Value(1.0), Value("One"), Value(2.0), Value("Two"), Value(3.0), Value("Three")});
...
TEST_F(SwitchFunctionTest, NoMatch_WithDefault_ReturnsDefault) {
    auto result = callSwitch({Value(4.0), Value(1.0), Value("One"), Value(2.0), Value("Two"), Value("Default")});
    ASSERT_TRUE(result.isText());
...
TEST_F(SwitchFunctionTest, TextMatch_CaseSensitive) {
    auto result = callSwitch({Value("hello"), Value("hello"), Value("Match"), Value("Hello"), Value("NoMatch")});
    ASSERT_TRUE(result.isText());
    EXPECT_EQ("Match", result.asText());
    
    result = callSwitch({Value("Hello"), Value("hello"), Value("Match"), Value("Hello"), Value("NoMatch")});
    ASSERT_TRUE(result.isText());
    EXPECT_EQ("NoMatch", result.asText());
}

TEST_F(SwitchFunctionTest, BooleanMatch_ReturnsCorrectResult) {
    auto result = callSwitch({Value(true), Value(false), Value("False"), Value(true), Value("True")});
    ASSERT_TRUE(result.isText());
...
TEST_F(SwitchFunctionTest, ErrorMatching_WithNonErrorValues_WorksCorrectly) {
    ...
    auto result = callSwitch({Value::error(ErrorType::DIV_ZERO), 
                              Value(1.0), Value("One"),
                              Value("Default")});
    ASSERT_TRUE(result.isText());
    EXPECT_EQ("Default", result.asText());
...
TEST_F(SwitchFunctionTest, EmptyTextMatch_WorksCorrectly) {
    auto result = callSwitch({Value(""), Value(""), Value("Empty"), Value("text"), Value("NotEmpty")});
    ASSERT_TRUE(result.isText());
...
TEST_F(SwitchFunctionTest, MultipleValuePairs_FindsCorrectMatch) {
    auto result = callSwitch({Value("B"), 
                              Value("A"), Value("First"),
