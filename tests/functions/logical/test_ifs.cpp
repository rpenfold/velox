...
TEST_F(IfsFunctionTest, ErrorCondition_PropagatesError) {
    auto result = callIfs(
            {Value::error(ErrorType::DIV_ZERO), Value("Error"), Value(true), Value("True")});
    EXPECT_TRUE(result.isError());
...
TEST_F(IfsFunctionTest, ErrorInResult_PropagatesOnMatch) {
    auto result = callIfs(
            {Value(true), Value::error(ErrorType::VALUE_ERROR), Value(false), Value("False")});
    EXPECT_TRUE(result.isError());
    EXPECT_EQ(ErrorType::VALUE_ERROR, result.asError());
}
...
TEST_F(IfsFunctionTest, MultipleConditions_ReturnsFirstTrue) {
    auto result = callIfs({Value(false), Value("First"), Value(true), Value("Second"), Value(true),
                           Value("Third")});
    ASSERT_TRUE(result.isText());
    EXPECT_EQ("Second", result.asText());
}
...
TEST_F(IfsFunctionTest, FloatingPointNumbers_WorkCorrectly) {
    auto result = callIfs({Value(0.0), Value("Zero"), Value(0.1), Value("PointOne"), Value(false),
                           Value("False")});
    ASSERT_TRUE(result.isText());
