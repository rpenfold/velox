...
TEST_F(ProperFunctionTest, AlreadyProperCase_ReturnsSame) {
    auto result = callProper({Value("Hello")});
...
TEST_F(ProperFunctionTest, AllUppercase_ConvertsToProperCase) {
    auto result = callProper({Value("HELLO")});
...
TEST_F(ProperFunctionTest, MixedCase_ConvertsToProperCase) {
    auto result = callProper({Value("hElLo")});
...
TEST_F(ProperFunctionTest, MultipleWords_CapitalizesEachWord) {
    auto result = callProper({Value("hello world test")});

    EXPECT_TRUE(result.isText());
    EXPECT_EQ("Hello World Test", result.asText());
}

TEST_F(ProperFunctionTest, SingleWord_OnlyCapitalizesFirst) {
    auto result = callProper({Value("hello")});
