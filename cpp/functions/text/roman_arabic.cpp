...
static std::string toRomanInt(long long num) {
    ...
    for (const auto& it : items) {
        while (num >= it.val) { out += it.sym; num -= it.val; }
    }
    return out;
}
...
static long long fromRoman(const std::string& s) {
    ...
    long long total = 0; long long prev = 0;
...
Value roman(const std::vector<Value>& args, const Context& context) {
    (void)context;
    auto err = utils::validateArgCount(args, 1, "ROMAN"); if (!err.isEmpty()) return err;
    auto n = utils::toNumberSafe(args[0], "ROMAN"); if (n.isError()) return n;
    long long v = static_cast<long long>(n.asNumber());
    if (v <= 0) return Value::error(ErrorType::NUM_ERROR);
...
Value arabic(const std::vector<Value>& args, const Context& context) {
    (void)context;
    auto err = utils::validateArgCount(args, 1, "ARABIC"); if (!err.isEmpty()) return err;
