#include "xl-formula/functions.h"

namespace xl_formula {
namespace functions {
namespace builtin {

Value lcm(const std::vector<Value>& args, const Context& context) {
    (void)context;

    // LCM requires at least 1 argument
    if (args.empty()) {
        return Value::error(ErrorType::VALUE_ERROR);
    }

    // Check for errors first
    auto errorCheck = utils::checkForErrors(args);
    if (!errorCheck.isEmpty()) {
        return errorCheck;
    }

    // Convert all arguments to numbers
    std::vector<double> numbers;
    for (const auto& arg : args) {
        auto num = utils::toNumberSafe(arg, "LCM");
        if (num.isError()) {
            return num;
        }
        numbers.push_back(num.asNumber());
    }

    // Helper functions
    auto gcd_two = [](long long a, long long b) -> long long {
        a = std::abs(a);
        b = std::abs(b);
        while (b != 0) {
            long long temp = b;
            b = a % b;
            a = temp;
        }
        return a;
    };

    auto lcm_two = [&gcd_two](long long a, long long b) -> long long {
        if (a == 0 || b == 0) {
            return 0;
        }
        return std::abs(a * b) / gcd_two(a, b);
    };

    long long result = static_cast<long long>(std::abs(numbers[0]));
    for (size_t i = 1; i < numbers.size(); ++i) {
        long long val = static_cast<long long>(std::abs(numbers[i]));
        result = lcm_two(result, val);
    }

    return Value(static_cast<double>(result));
}

}  // namespace builtin
}  // namespace functions
}  // namespace xl_formula