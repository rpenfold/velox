...
Value datevalue(const std::vector<Value>& args, const Context& context) {
    ...
    if (s.empty()) return Value::error(ErrorType::VALUE_ERROR);
    // Parse common forms:
    ...
    char sep1 = 0, sep2 = 0;
    {
        ...
        if (!(iss >> part1)) return Value::error(ErrorType::VALUE_ERROR);
        if (!(iss >> sep1)) return Value::error(ErrorType::VALUE_ERROR);
        if (!(iss >> part2)) return Value::error(ErrorType::VALUE_ERROR);
        if (!(iss >> sep2)) return Value::error(ErrorType::VALUE_ERROR);
        if (!(iss >> part3)) return Value::error(ErrorType::VALUE_ERROR);
    ...
    if (isYear(part1)) {
        ...
        if (sep1 == '/') { m = part1; d = part2; y = part3; }
        else { d = part1; m = part2; y = part3; }
    } else {
        return Value::error(ErrorType::VALUE_ERROR);
    }
    if (m < 1 || m > 12 || d < 1 || d > 31) return Value::error(ErrorType::VALUE_ERROR);
    std::tm out{}; out.tm_year = y - 1900; out.tm_mon = m - 1; out.tm_mday = d;
    out.tm_hour = 0; out.tm_min = 0; out.tm_sec = 0; out.tm_isdst = -1;
    auto out_tt = std::mktime(&out);
    if (out_tt == -1) return Value::error(ErrorType::VALUE_ERROR);
    return Value(std::chrono::system_clock::from_time_t(out_tt));
}
...
Value timevalue(const std::vector<Value>& args, const Context& context) {
    ...
    if (sraw.empty()) return Value::error(ErrorType::VALUE_ERROR);
    ...
    if (s.size() >= 2) {
        ...
        auto trim_end = [](std::string& str){ while(!str.empty() && std::isspace(static_cast<unsigned char>(str.back()))) str.pop_back(); };
        trim_end(up);
        if (up.size() >= 2) {
            if (up.size() >= 3 && up.substr(up.size()-3) == " AM") { hasAmPm = true; isPM = false; s = s.substr(0, s.size()-3); }
            else if (up.size() >= 3 && up.substr(up.size()-3) == " PM") { hasAmPm = true; isPM = true; s = s.substr(0, s.size()-3); }
            else if (up.substr(up.size()-2) == "AM") { hasAmPm = true; isPM = false; s = s.substr(0, s.size()-2); }
            else if (up.substr(up.size()-2) == "PM") { hasAmPm = true; isPM = true; s = s.substr(0, s.size()-2); }
        }
        s = trim_copy(s);
    }
    int hh=0, mm=0, ss=0; char c1=0, c2=0;
    std::istringstream iss(s);
    if (!(iss >> hh >> c1 >> mm) || c1 != ':') {
        return Value::error(ErrorType::VALUE_ERROR);
    }
    if (iss >> c2) {
        if (c2 == ':') {
            if (!(iss >> ss)) return Value::error(ErrorType::VALUE_ERROR);
        } else {
    ...
    if (hasAmPm) {
        if (hh < 1 || hh > 12) return Value::error(ErrorType::VALUE_ERROR);
        if (isPM) { if (hh != 12) hh += 12; }
        else { if (hh == 12) hh = 0; }
    }
    if (hh < 0 || hh > 23 || mm < 0 || mm > 59 || ss < 0 || ss > 59) return Value::error(ErrorType::VALUE_ERROR);
