...
/**
 * @brief Finds one text string within another (case-insensitive)
 * @ingroup text
...
Value search(const std::vector<Value>& args, const Context& context) {
    ...
    // SEARCH requires 2-3 arguments
    auto error = utils::validateMinArgs(args, 2, "SEARCH");
    if (!error.isEmpty()) {
        return error;
    }
    ...
    std::string find_text_lower = find_text;
    std::string within_text_lower = within_text;

    std::transform(find_text_lower.begin(), find_text_lower.end(), find_text_lower.begin(),
                   ::tolower);
    std::transform(within_text_lower.begin(), within_text_lower.end(), within_text_lower.begin(),
                   ::tolower);
